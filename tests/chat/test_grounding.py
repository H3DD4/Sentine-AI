...
class StructuredChatGroundingTests(unittest.TestCase):
    ...
    def test_exact_model_vector_without_analyst_vector_is_calculated(self):
        draft = _draft(cvss={
            "status": "exact",
            "version": "3.1",
    ...
    def test_model_proposed_range_without_current_user_vectors_is_calculated(self):
        ...
        self.assertIsNotNone(result.draft.cvss.lower_bound)
    ...
    def test_conversational_response_receives_and_renders_deterministic_cvss(self):
        client = AsyncMock()
        client.generate = AsyncMock(return_value="The demonstrated scenario is high severity.")
        vector = "CVSS:3.1/AV:N/AC:L/PR:L/UI:N/S:C/C:H/I:N/A:N"

        response, _ = asyncio.run(generate_conversational_response(
            client,
            [{"role": "user", "content": f"Use this lower scenario: {vector}"}],
            _outcome(),
        ))

        request = client.generate.await_args.kwargs["messages"][0]["content"]
        self.assertIn("DETERMINISTIC LEDGER (IMMUTABLE)", request)
        self.assertIn('"score": 7.7', request)
        self.assertIn('"severity": "high"', request)
        self.assertIn("Calculated by the deterministic CVSS MCP tool", response)
        self.assertNotIn("calculated by Python", response)

    def test_model_cvss_prose_cannot_override_deterministic_ledger(self):
        client = AsyncMock()
        client.generate = AsyncMock(return_value="The score is 8.2.")
        vector = "CVSS:3.1/AV:N/AC:L/PR:L/UI:N/S:C/C:H/I:N/A:N"

        response, _ = asyncio.run(generate_conversational_response(
            client,
            [{"role": "user", "content": f"Calculate this vector: {vector}"}],
            _outcome(),
        ))

        self.assertIn("7.7 High", response)
        self.assertNotIn("The score is 8.2", response)

    def test_conversational_tool_request_is_calculated_before_rendering(self):
        client = AsyncMock()
        client.generate = AsyncMock(side_effect=[
            '{"tool":"calculate_cvss","arguments":{"vector":"CVSS:3.1/AV:N/AC:L/PR:L/UI:N/S:C/C:H/I:N/A:N"}}',
            "The calculated result is authoritative.",
        ])
        calculation = {
            "status": "valid",
            "version": "3.1",
            "canonical_vector": "CVSS:3.1/AV:N/AC:L/PR:L/UI:N/S:C/C:H/I:N/A:N",
            "score": 7.7,
            "severity": "high",
        }

        with unittest.mock.patch(
            "app.services.chat_grounding.calculate_cvss_via_mcp",
            new=AsyncMock(return_value=calculation),
        ) as tool:
            response, _ = asyncio.run(generate_conversational_response(
                client,
                [{"role": "user", "content": "What is the CVSS score?"}],
                _outcome(),
            ))

        tool.assert_awaited_once()
        self.assertIn("7.7 High", response)
        self.assertIn("CVSS:3.1/AV:N", response)

    def test_empty_conversational_tool_request_returns_pending(self):
        client = AsyncMock()
        client.generate = AsyncMock(return_value='{"tool":"calculate_cvss","arguments":{}}')
    ...
    def test_requested_unretrieved_identifier_remains_visible_in_chat(self):
        client = AsyncMock()
        client.generate = AsyncMock(return_value=(
            "| Identifier | Status |\n|---|---|\n"
            "| CVE-2020-1938 | no exact record retrieved this turn |\n"
            "| CVE-2099-9999 | invented |"
        ))

        response, _ = asyncio.run(generate_conversational_response(
            client,
            [{"role": "user", "content": "Check CVE-2020-1938"}],
            _outcome(),
        ))

        self.assertIn("CVE-2020-1938", response)
        self.assertNotIn("CVE-2099-9999", response)

    def test_conversational_response_continues_when_provider_hits_length_limit(self):
        client = AsyncMock()
        parts = iter(["first half ", "second half"])

        async def generate(**_kwargs):
            part = next(parts)
            client.last_finish_reason = "length" if part.startswith("first") else "stop"
            return part

        client.generate.side_effect = generate
        response, _ = asyncio.run(generate_conversational_response(
            client,
            [{"role": "user", "content": "Write a complete report"}],
            _outcome(),
        ))

        self.assertEqual(response, "first half second half")
        self.assertEqual(client.generate.await_count, 2)
        continuation = client.generate.await_args_list[1].kwargs["messages"][-1]["content"]
        self.assertIn("Continue exactly where you stopped", continuation)
    ...
    def test_model_proposed_vector_is_calculated_when_current_turn_has_none(self):
        vector = "CVSS:3.1/AV:N/AC:L/PR:L/UI:N/S:C/C:H/I:H/A:N"
        client = AsyncMock()
    ...
    def test_malformed_completions_keep_deterministic_evidence_summary(self):
        ...
        self.assertIn("Incomplete analysis", response)
        self.assertIn("LLM analysis is unavailable", response)
        self.assertIn("Pending evidence", response)
    ...
    def test_command_injection_fallback_describes_supplied_command_output(self):
        ...
        self.assertIn("LLM analysis is unavailable", result.answer_markdown)
        self.assertNotIn("command injection", result.answer_markdown.lower())
        self.assertNotIn("uid=1001(app)", result.answer_markdown.lower())
